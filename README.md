...
# TP files de priorité (version beta)
...
Vous allez récupérer les sources du projet via [git](https://git-scm.com/) et vous trouverez [ici](https://moodle.insa-rennes.fr/mod/resource/view.php?id=26679) une documentation vous expliquant les différentes étapes à suivre.
...
au constructeur à la ligne 11, mais s'il ne le fournit pas, on utilise le comparateur du type `T` à la ligne 17^[C'est la façon dont c'est réalisé dans la classe `PriorityQueue` de Java.].
...
L'idée du recadrage intelligent est assez simple, et va utiliser une recherche de plus court chemin suivant un critère particulier pour trouver les pixels à supprimer dans l'image.
